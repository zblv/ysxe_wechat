								...
								<tbody>
									...
									<c:forEach items="${wechatDataAudits}" var="bean">
										<tr ondblclick="showData(${bean.id },'${bean.cusType }','${bean.relName }','${bean.mobilePhone }','${bean.email }','${bean.address}','${bean.loanMoney}','${bean.loanTerm}','${bean.loanPurpose}','${bean.repaySource}','${bean.isMortgage}','${bean.mortgageType}','${bean.state}','${bean.auditPeople}','${bean.remark }','<fmt:formatDate value="${bean.createDate }" pattern="yyyy-MM-dd HH:mm:ss"/>','<fmt:formatDate value="${bean.updateDate }" pattern="yyyy-MM-dd HH:mm:ss"/>');">
											...
											<td>
						               		<a class='btn btn-danger' href='javascript:delData(${bean.id});' title="删除">
						                   		<i class='icon-remove'>&nbsp;&nbsp;删&nbsp;&nbsp;除</i>
						               		</a>
							           		</td>
										</tr>
	...
	<div class='box' style="margin-bottom: 0px;">
		...
		<div class='box-content'>
			...
			<div class='control-group'>
				<label class='control-label' for='cusType'>客户类型</label>
				<div class='controls'>
					<input id='hcusType' name='cusType' placeholder='客户类型' />
				</div>
			</div>
			<div class='control-group'>
				<label class='control-label' for='relName'>客户姓名</label>
				<div class='controls'>
					<input id='hrelName' name='relName' placeholder='客户姓名' />
				</div>
			</div>
			<div class='control-group'>
				<label class='control-label' for='mobilePhone'>手机号</label>
				<div class='controls'>
					<input id='hmobilePhone' name='mobilePhone' placeholder='手机号' />
				</div>
			</div>
			<div class='control-group'>
			...
			<div class='control-group'>
				...
				<div class='controls'>
					<input id='haddress' name='address' placeholder='联系地址' />
				</div>
			</div>
			<div class='control-group'  >
				<label class='control-label' for='loanMoney'>贷款金额（单位：万）</label>
				<div class='controls'>
					<input id='hloanMoney' name='loanMoney' placeholder='贷款金额（单位：万）'/>
				</div>
			</div>
			<div class='control-group' >
				<label class='control-label' for='loanTerm'>贷款期限（单位：月）</label>
				<div class='controls'>
					<input id='hloanTerm' name='loanTerm' placeholder='贷款期限（单位：月）' />
				</div>
			</div>
			<div class='control-group' >
				<label class='control-label' for='loanPurpose'>借款用途</label>
				<div class='controls'>
					<input id='hloanPurpose' name='loanPurpose' placeholder='借款用途' />
				</div>
			</div>
			<div class='control-group' >
				<label class='control-label' for='repaySource'>还款来源</label>
				<div class='controls'>
					<input id='hrepaySource' name='repaySource' placeholder='还款来源' />
				</div>
			</div>
			<div class='control-group' >
				<label class='control-label' for='isMortgage'>有无抵押</label>
				<div class='controls'>
					<input id='hisMortgage' name='isMortgage' placeholder='有无抵押' />
				</div>
			</div>
			<div class='control-group' >
				<label class='control-label' for='mortgageType'>抵押类型</label>
				<div class='controls'>
					<input id='hmortgageType' name='mortgageType' placeholder='抵押类型' />
				</div>
			</div>
			<div class='control-group'>
                <label class='control-label' for='remark'>备注</label>
                <div class='controls'>
                    <textarea class='input-block-level' id='hremark' name="remark" placeholder='备注' rows='2'></textarea>
                </div>
            </div>
	            <div class='control-group' >
			...
			<div class='form-actions' style='margin-bottom: 0'>
				<button class='btn btn-primary'
					onclick="updateData();return false;">
...
<script type="text/javascript">
	...
	//禁用或启用管理员
	function delOrOpenData(id, state) {
		$.ajax({
			url : "${basePath}system/data/updateDataState",
			data : {
				id : id,
				state : state
			},
			...
			success : function(data) {
				if (data.status == "SUCCESS") {
					window.location.href="${basePath}system/data/shInit?fMenuUrl="+encodeURI(encodeURI("system/admin/init"))+"&currMenuName="+encodeURI(encodeURI("资料审核管理"))+"&fMenuName="+encodeURI(encodeURI("资料审核"))+"";
				} else {
					alert(data.message);
				}
			}
	...
	function showData(id, cusType,relName,mobilePhone,email,address,loanMoney,loanTerm,
				loanPurpose,repaySource,isMortgage,mortgageType,state,
				auditPeople,remark,createDate,updateDate) {
		$("#hid").val(id);
		$("#hcusType").val(cusType);
	...
	function updateData(){
		...
		$.ajax({
			url : "${basePath}/system/data/updateData",
			data : $("form").serialize(),
			type : "post",
			async : false,
			dataType : "json",
	...
	function delData(id){
			if(confirm("确认要删除？")){ 
				$.ajax({ 
					type : "post", 
					url : "${basePath}/system/data/delData?id=" + id, 
					success : function(data) { 
						if (data.status == "SUCCESS") {
							window.location.href="${basePath}system/data/shInit?fMenuUrl="+encodeURI(encodeURI("system/admin/init"))+"&currMenuName="+encodeURI(encodeURI("资料审核管理"))+"&fMenuName="+encodeURI(encodeURI("资料审核"))+"";
						} else {
